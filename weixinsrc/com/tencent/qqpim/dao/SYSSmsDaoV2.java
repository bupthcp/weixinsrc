...
public class SYSSmsDaoV2 extends SYSSmsDao
{
    ...
    private SYSSmsDaoV2(Context context1)
    {
        super(context1);
        smsUri = null;
        idColumnIndex = -1;
...
_L11:
    ...
    protected static SYSSmsDaoV2 getInstance(Context context1)
    {
        if(sysSmsDaoV2 == null)
            sysSmsDaoV2 = new SYSSmsDaoV2(context1);
        return sysSmsDaoV2;
    }
...
_L2:
        ...
        boolean flag1;
        builder = ContentProviderOperation.newInsert(smsUri).withYieldAllowed(flag);
        builder.withValue("read", "1");
        if(QQPimUtils.isSDKVersionLargerOrEquals2_2() && hasColumnSeen)
            builder.withValue("seen", "1");
        flag1 = false;
_L5:
label0:
        {
            ...
            if(!flag1)
                builder.withValue("address", null);
            if(ientity.getId() != null && !ientity.getId().equals(""))
                builder.withValue("_id", ientity.getId());
...
_L3:
        ...
        if(record != null)
        {
            ...
            } else
            {
                if(record.get(0).equals("FOLDER"))
                {
                    integer = (Integer)vcardTypeToDbType.get(s);
                    if(integer == null)
                        integer = Integer.valueOf(flag);
                    builder.withValue("type", integer.toString());
                    i = ((flag1) ? 1 : 0);
                } else
                if(record.get(0).equals("SENDER"))
                {
                    if(s == null || s.equals(""))
                        ...
                        builder.withValue("address", s);
                    i = ((flag) ? 1 : 0);
                } else
                if(record.get(0).equals("SENDDATE"))
                {
                    builder.withValue("date", Long.valueOf(QQPimUtils.getUTCTimeFromString(s)).toString());
                    i = ((flag1) ? 1 : 0);
                } else
                {
                    if(record.get(0).equals("INFORMATION"))
                        builder.withValue("body", s);
                    i = ((flag1) ? 1 : 0);
                }
                ientity.moveToNext();
                flag1 = i;
...
_L1:
        ContentProviderResult acontentproviderresult[] = contentResolver.applyBatch(smsUri.getAuthority(), arraylist2);
        int k = 0;
_L7:
